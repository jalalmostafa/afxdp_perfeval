...
require "Pktgen";
local DEBUG                        = true;
local USE_RAMPUP                   = true; --Ramp up traffic flow before starting measurement, less accurate measurement (trial time,drops).
...
-- define packet sizes to test: 64, 128, 256, 512, 1024, 1280, 1518, 3498
local pkt_sizes                    = { 64, 128, 256, 512, 1024, 1280, 1518, 3498 };
...
local pktgen_limit                 = {
    [64]   = { ["rate"] = 100 }, -- 68% eq 200MPPS on 2x100GbE	
    [128]  = { ["rate"] = 100 },
    [256]  = { ["rate"] = 100 },
    [512]  = { ["rate"] = 100 },
    [1024] = { ["rate"] = 100 },
...
local latency_sleep                = 1;
local assess_throughput_iterations = 0;
local latency_iterations           = 10;
...
local num_ip                       = 1; -- Number of IPs per interface, max 253. Set DUT IP at .254.
local num_udpp                     = 1; -- Number of UDP ports used per IP, max 9999.
local srcip                        = "192.168.20.1";
local max_srcip                    = "192.168.20." .. num_ip;
local dstip                        = "192.168.20.2";
local max_dstip                    = "192.168.20." .. num_ip;
local netmask                      = "/24";
local p0_udpp_src                  = 10000;
local p0_udpp_dst                  = 8877;
local p1_udpp_src                  = p0_udpp_src;
local p1_udpp_dst                  = p0_udpp_dst;
local p0_destmac                   = "1c:34:da:5b:a7:d4"; -- Comment out to test Pktgen loopback
local p1_destmac                   = p0_destmac;          -- Set to correct DUT MACs
...
local loss_limit                   = 0;
local adjust_rate                  = 0;
local same_ports                   = sendport == recvport;
...
local function getLinkSpeed()
    local port_link = string.match(pktgen.linkState(sendport)[tonumber(sendport)], "%d+");
...
local function checkRate(pkt_size, set_rate, duration, num_pkts)
    ...
    dprint("[ DEBUG ] Link:" ..
        (getLinkSpeed() // 1000) .. "GBE | Max MPPS:" .. Round(max_pps / 10 ^ 6, 2) .. " | Test MPPS:" ..
        Round(calc_pps / 10 ^ 6, 2) .. " | Dur: " .. duration .. " | Calc Rate:" .. test_rate .. " < " ..
        set_rate .. ":Set Rate");

    if (test_rate - set_rate) < -1 then
        logResult("[WARNING] Set rate " ..
            set_rate .. " is not reached (" .. test_rate .. " < " .. set_rate .. "), PktGen is unable to reach Tx target");
...
local function runTrial(pkt_size, rate, duration, count, retry)
    ...
    if (num_tx + 5000) < num_tx1 then
        print("[WARNING] Port:" ..
            sendport .. " Tx underruns. Results might be inconclusive.(" .. Round(num_tx - num_tx1) .. ")");
        dprint("[ DEBUG ] CPU probably overloaded.");
    elseif (num_tx1 + 5000) < num_tx then
        print("[WARNING] Port:" ..
            recvport .. " Tx underruns. Results might be inconclusive.(" .. Round(num_tx1 - num_tx) .. ")");
        dprint("[ DEBUG ] CPU probably overloaded.");
    else
        dprint("[ DEBUG ] No Tx underruns are detected.");
    end

    num_tx = same_ports and num_tx or (num_tx + num_tx1);
    num_rx = same_ports and statRx.ipackets or (statRx.ipackets + statTx.ipackets);
    ...
    if checkRate(pkt_size, rate, duration, num_tx) then
        rate_notif = " ";
    else
    ...
    logResult(sprintf(
        "[ %4dB ] Trial %5s.%1d   | Rate:%s %-6.2f%%  | Pkt Size: %4dB | Dur.: %3.2fs | Loss Tol.: %-5.3f%%", pkt_size,
        tostring(count), retry, rate_notif, rate, pkt_size, Round(duration / 1000, 3), loss_tol));
    logResult(sprintf("[ %4dB ] Tx: %-11d | Rx: %-11d | Drop: %-13d (%-6.3f%%)  | Loss Lim.: %-10.0f", pkt_size, num_tx,
        num_rx, num_dropped, loss, loss_limit));
    logResult(sprintf(
        "[ %4dB ] MPPS Tx: %-06.2f | MPPS Rx: %-06.2f | Gbps Tx L1/L2: %-6.2f / %-6.2f | Gbps Rx L1/L2: %-6.2f / %-6.2f",
        pkt_size, tx_mpps, rx_mpps, tx_gbps1, tx_gbps2, rx_gbps1, rx_gbps2));
...
local function runThroughputTest(pkt_size)
    ...
    for count = 1, numIterations, 1 do
        ...
        if num_dropped <= loss_limit then
            -- Undershoot
            if no_adjust then
                min_rate = trial_rate;
            end
        ...
        if max_min_diff <= seekPrec then
            dprint("[ DEBUG ] Stopping seek iterations, reached seek Precision limit");
            break
        end
    end

    if assess_throughput_iterations > 0 then
        for count = 1, assess_throughput_iterations, 1 do
            num_dropped = runTrial(pkt_size, trial_rate, duration, "Assess", count);
        end
    end
    ...
    if confirmDuration > 0 then
        ...
        printLine();
        if num_dropped <= loss_limit then
            local result = "[RESULT ] Max rate for packet size " .. pkt_size .. "B is: " .. adjust_rate .. "%.";
            print(result);
            file:write(result .. "\n\n");
        else
            local warning = "[WARNING] Max rate of " ..
                trial_rate ..
                "% could not be confirmed for 60 seconds as required by rfc2544. Dropped: " ..
                tostring(num_dropped) .. " > " .. tostring(loss_limit) .. " :Loss Limit"
            file:write(warning .. "\n\n");
            print(warning);
        end
        file:flush();
...
local function getLatency(a)
    ...
    if a.pktsize == 0 then a.pktsize = 64 end
    if a.sleeptime == 0 then a.sleeptime = 2 end

    pktgen.set(a.sendport, "count", 0)
    pktgen.set(a.sendport, "rate", a.rate / 2)
    pktgen.set(a.sendport, "size", a.pktsize)

    -- enable latency
    pktgen.page("latency");
    pktgen.latency(a.sendport, "enable");
    local lat_rate = (a.sleeptime / 2) * 1e6;
    pktgen.latency(a.sendport, "rate", lat_rate);
    -- pktgen.latency(a.sendport, "entropy", 12);

    if a.sendport ~= a.recvport then
    ...
    for i = 1, a.iterations, 1 do
        ...
        min_cycles = Round(port_stats[tonumber(a.recvport)].latency.min_cycles, 4);
        avg_cycles = Round(port_stats[tonumber(a.recvport)].latency.avg_cycles, 4);
        max_cycles = Round(port_stats[tonumber(a.recvport)].latency.max_cycles, 4);
        min_us = Round(port_stats[tonumber(a.recvport)].latency.min_us, 4);
        avg_us = Round(port_stats[tonumber(a.recvport)].latency.avg_us, 4);
        max_us = Round(port_stats[tonumber(a.recvport)].latency.max_us, 4);
        ...
        local stat = table.concat({ t1, num_lat_pkts, min_cycles, avg_cycles,
            max_cycles, min_us, avg_us, max_us, mbits_rx, mbits_tx, num_skipped }, "    ");
        logResult("[  INFO ] " .. stat)
    end
...
local function runLatencyTest(rate, size)
    printLine();
    logResult("[  INFO ] Running latency test for packet size " ..
        tostring(size) .. " at rate " .. tostring(rate) .. "%.");
    local headers = table.concat({ "time", "nbPkts", "minCycles", "avgCycles",
        "maxCycles", "min_us", "avg_us", "max_us",
        "RxMB", "TxMB", "Skipped" }, "    ");
    logResult("[  INFO ] " .. headers)
    pktgen.reset("all")
    ...
    getLatency {
        ...
        rate = rate,
        pktsize = size,
        sleeptime = latency_sleep,
        iterations = latency_iterations
    }
    logResult("[  INFO ] End of latency tests");
    printLine();
end
...
function main()
    ...
    if dummy >= (sent * loss_tol) then
        ...
        for _, size in pairs(pkt_sizes) do
            local rate = runThroughputTest(size);
            if RUN_LATENCY then
                runLatencyTest(rate, size)
            end
