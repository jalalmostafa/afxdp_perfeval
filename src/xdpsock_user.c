...
static void dump_app_stats(long dt)
{
    ...
    for (i = 0; i < num_socks && xsks[i]; i++) {
        ...
        rx_empty_polls_ps = (xsks[i]->app_stats.rx_empty_polls - xsks[i]->app_stats.prev_rx_empty_polls) * 1000000000. / dt;
        fill_fail_polls_ps = (xsks[i]->app_stats.fill_fail_polls - xsks[i]->app_stats.prev_fill_fail_polls) * 1000000000. / dt;
        copy_tx_sendtos_ps = (xsks[i]->app_stats.copy_tx_sendtos - xsks[i]->app_stats.prev_copy_tx_sendtos) * 1000000000. / dt;
        tx_wakeup_sendtos_ps = (xsks[i]->app_stats.tx_wakeup_sendtos - xsks[i]->app_stats.prev_tx_wakeup_sendtos)
            * 1000000000. / dt;
    ...
    for (i = 0; i < num_socks && xsks[i]; i++) {
        ...
        if (opt_extra_stats) {
            if (!xsk_get_xdp_stats(xsk_socket__fd(xsks[i]->xsk), xsks[i])) {
                dropped_pps = (xsks[i]->ring_stats.rx_dropped_npkts - xsks[i]->ring_stats.prev_rx_dropped_npkts) * 1000000000. / dt;
                rx_invalid_pps = (xsks[i]->ring_stats.rx_invalid_npkts - xsks[i]->ring_stats.prev_rx_invalid_npkts) * 1000000000. / dt;
                tx_invalid_pps = (xsks[i]->ring_stats.tx_invalid_npkts - xsks[i]->ring_stats.prev_tx_invalid_npkts) * 1000000000. / dt;
                full_pps = (xsks[i]->ring_stats.rx_full_npkts - xsks[i]->ring_stats.prev_rx_full_npkts) * 1000000000. / dt;
                fill_empty_pps = (xsks[i]->ring_stats.rx_fill_empty_npkts - xsks[i]->ring_stats.prev_rx_fill_empty_npkts) * 1000000000. / dt;
                tx_empty_pps = (xsks[i]->ring_stats.tx_empty_npkts - xsks[i]->ring_stats.prev_tx_empty_npkts) * 1000000000. / dt;
...
static void gen_eth_hdr_data(void)
{
    ...
    if (opt_vlan_tag) {
        ...
        pktgen_hdr = (struct pktgen_hdr*)(pkt_data + sizeof(struct vlan_ethhdr) + sizeof(struct iphdr) + sizeof(struct udphdr));
...
static void parse_command_line(int argc, char** argv)
{
    ...
    for (;;) {
        ...
        case 's':
            ...
            if (opt_pkt_size > (XSK_UMEM__DEFAULT_FRAME_SIZE) || opt_pkt_size < MIN_PKT_SIZE) {
                fprintf(stderr,
    ...
    int batch_size, unsigned long tx_ns)
{
    ...
    while (xsk_ring_prod__reserve(&xsk->tx, batch_size, &idx) < (u32)batch_size) {
...
static inline int get_batch_size(int pkt_cnt)
{
    ...
    if (pkt_cnt + opt_batch_size <= (u32)opt_pkt_count)
        return opt_batch_size;
...
int main(int argc, char** argv)
{
    ...
    dlog("rx_dropped (other): %lld, rx_invalid_descs: %lld, rx_ring_full: %lld, rx_fill_ring_empty_descs: %lld\n",
